           ...
           do i=1,nx
               if(isfluid(i,j).eq.1)then
                 ...
                 elseif(isfluid(i,j).eq.0)then
                     f1(i+1,j)=p(3)*rho(i,j)
                     f3(i-1,j)=p(1)*rho(i,j)
                     f2(i,j+1)=p(4)*rho(i,j)
                     f4(i,j-1)=p(2)*rho(i,j)
                     f5(i+1,j+1)=p(7)*rho(i,j)
                     f7(i-1,j-1)=p(5)*rho(i,j)
                     f6(i-1,j+1)=p(8)*rho(i,j)
                     f8(i+1,j-1)=p(6)*rho(i,j)
                endif
            enddo
        enddo
        !!$acc end kernels
