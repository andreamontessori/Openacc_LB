        ...
        do j=1,ny
            do i=1,nx
                if(isfluid(i,j).eq.1.or.isfluid(i,j).eq.0)then
                    ...
                    fneq1=f1(i,j)-p(1)*(rho(i,j)+rho(i,j)*(temp + udotc))
                    fneq3=f3(i,j)-p(3)*(rho(i,j)+rho(i,j)*(temp - udotc))
                    !2-4
                    udotc=v(i,j)/cssq
                    temp = -uu + 0.5_db*udotc*udotc
                    fneq2=f2(i,j)-p(2)*(rho(i,j)+rho(i,j)*(temp + udotc))
                    fneq4=f4(i,j)-p(4)*(rho(i,j)+rho(i,j)*(temp - udotc))
                    !5-7
                    udotc=(u(i,j)+v(i,j))/cssq
                    temp = -uu + 0.5_db*udotc*udotc
                    fneq5=f5(i,j)-p(5)*(rho(i,j)+rho(i,j)*(temp + udotc))
                    fneq7=f7(i,j)-p(7)*(rho(i,j)+rho(i,j)*(temp - udotc))
                    !6-8
                    udotc=(-u(i,j)+v(i,j))/cssq
                    temp = -uu + 0.5_db*udotc*udotc
                    fneq6=f6(i,j)-p(6)*(rho(i,j)+rho(i,j)*(temp + udotc))
                    fneq8=f8(i,j)-p(8)*(rho(i,j)+rho(i,j)*(temp - udotc))
        ...
        do j=1,ny
           do i=1,nx   
                ...
                if(mod_psi>0.001)then ! i'm sitting on the interface 
                    ...
                    !recoloring
                    gaddendum1=0.25*rho(i,j)*beta*p(1)*(1.0_db-psi(i,j)**2)*psi_x/mod_psi
                ...
                udotc=(u(i,j)+v(i,j))/cssq
                temp = -uu + 0.5_db*udotc*udotc
                feq=p(5)*(rho(i,j)+rho(i,j)*(temp + udotc))
                ...
                f5(i+1,j+1)=fpc
                !7
                feq=p(7)*(rho(i,j)+rho(i,j)*(temp - udotc))
                ...
                udotc=(-u(i,j)+v(i,j))/cssq
                temp = -uu + 0.5_db*udotc*udotc
                feq=p(6)*(rho(i,j)+rho(i,j)*(temp + udotc))
                ...
                f6(i-1,j+1)= fpc
                !8
                feq=p(8)*(rho(i,j)+rho(i,j)*(temp - udotc))
