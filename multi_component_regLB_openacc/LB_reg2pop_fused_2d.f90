...
!#endif
            ...
            do i=1,nx
                if(isfluid(i,j).eq.1.or.isfluid(i,j).eq.0)then
                    ...
                    if(isfluid(i,j).eq.0)then
                        ...
                        f0(i,j)=((p(0)*rtot)  + pi2cssq0*(-cssq*pxx(i,j)-cssq*pyy(i,j)))*rhoA(i,j)/rtot
                        f1(i,j)=((p(3)*rtot)  + pi2cssq1*(qxx*pxx(i,j)-cssq*pyy(i,j)))*rhoA(i,j)/rtot
                        f3(i,j)=((p(1)*rtot)  + pi2cssq1*(qxx*pxx(i,j)-cssq*pyy(i,j)))*rhoA(i,j)/rtot
                        f2(i,j)=((p(4)*rtot)  + pi2cssq1*(qyy*pyy(i,j)-cssq*pxx(i,j)))*rhoA(i,j)/rtot
                        f4(i,j)=((p(2)*rtot)  + pi2cssq1*(qyy*pyy(i,j)-cssq*pxx(i,j)))*rhoA(i,j)/rtot
                        f5(i,j)=((p(7)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy5_7*pxy(i,j)))*rhoA(i,j)/rtot
                        f7(i,j)=((p(5)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy5_7*pxy(i,j)))*rhoA(i,j)/rtot
                        f6(i,j)=((p(8)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy6_8*pxy(i,j)))*rhoA(i,j)/rtot
                        f8(i,j)=((p(6)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy6_8*pxy(i,j)))*rhoA(i,j)/rtot
                        !
                        g0(i,j)=((p(0)*rtot)  + pi2cssq0*(-cssq*pxx(i,j)-cssq*pyy(i,j)))*rhob(i,j)/rtot
                        g1(i,j)=((p(3)*rtot)  + pi2cssq1*(qxx*pxx(i,j)-cssq*pyy(i,j)))*rhob(i,j)/rtot
                        g3(i,j)=((p(1)*rtot)  + pi2cssq1*(qxx*pxx(i,j)-cssq*pyy(i,j)))*rhob(i,j)/rtot
                        g2(i,j)=((p(4)*rtot)  + pi2cssq1*(qyy*pyy(i,j)-cssq*pxx(i,j)))*rhob(i,j)/rtot
                        g4(i,j)=((p(2)*rtot)  + pi2cssq1*(qyy*pyy(i,j)-cssq*pxx(i,j)))*rhob(i,j)/rtot
                        g5(i,j)=((p(7)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy5_7*pxy(i,j)))*rhob(i,j)/rtot
                        g7(i,j)=((p(5)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy5_7*pxy(i,j)))*rhob(i,j)/rtot
                        g6(i,j)=((p(8)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy6_8*pxy(i,j)))*rhob(i,j)/rtot
                        g8(i,j)=((p(6)*rtot) + pi2cssq2*(qxx*pxx(i,j)+qyy*pyy(i,j)+2.0_db*qxy6_8*pxy(i,j)))*rhob(i,j)/rtot
                    endif
                endif
    ...
    do j=1,ny
        do i=1,nx
            !write(101,*) rhoB(i,j)+rhoA(i,j)!sqrt(u(i,j)**2+v(i,j)**2)  
        enddo
    enddo
    close(101)
end program