    ...
    do step=1,nsteps 
        ...
        do j=2,ny-1
           do i=2,nx-1
                nci(i,j)=0
                loc(i,j)=0
                psi_x(i,j)=(1.0_db/cssq)*(p(1)*(psi(i+1,j)-psi(i-1,j)) + p(5)*(psi(i+1,j+1) + psi(i+1,j-1)-psi(i-1,j+1)-psi(i-1,j-1)))
                psi_y(i,j)=(1.0_db/cssq)*(p(1)*(psi(i,j+1)-psi(i,j-1)) + p(5)*(psi(i+1,j+1) - psi(i+1,j-1)+psi(i-1,j+1)-psi(i-1,j-1)))
                mod_psi=sqrt(psi_x**2+psi_y**2)
                if(mod_psi>0.1)then
                    nci(i,j)=1
                    psi_x(i,j)=psi_x(i,j)/mod_psi
                    psi_y(i,j)=psi_y(i,j)/mod_psi
                endif
           enddo
        enddo
        ...
        do j=1,ny
           do i=1,nx   
                !oneminusuu= -uu !1.0_db - u
                !chromodynamic
                mod_psi=sqrt(psi_x(i,j)**2+psi_y(i,j)**2)
                mod_psi_sq=psi_x(i,j)**2+psi_y(i,j)**2 
                rr2=rho(i,j)*(1.0_db-psi(i,j))*0.5_db
                rr1=rho(i,j)-rr2
                nu_avg=1.0_db/((rr1/rho(i,j))*one_ov_nu1 + (rr2/rho(i,j))*one_ov_nu2)
                ...
                if(mod_psi>0.001)then ! i'm sitting on the interface 
                    ...
                    addendum1=st_coeff*mod_psi*(p(1)*psi_x(i,j)**2/mod_psi_sq - b1)
                    addendum2=st_coeff*mod_psi*(p(2)*psi_y(i,j)**2/mod_psi_sq - b1)
                    addendum3=st_coeff*mod_psi*(p(3)*psi_x(i,j)**2/mod_psi_sq - b1)
                    addendum4=st_coeff*mod_psi*(p(4)*psi_y(i,j)**2/mod_psi_sq - b1)
                    addendum5=st_coeff*mod_psi*(p(5)*(psi_x(i,j)+psi_y(i,j))**2/mod_psi_sq - b2)
                    addendum6=st_coeff*mod_psi*(p(6)*(-psi_x(i,j)+psi_y(i,j))**2/mod_psi_sq - b2)
                    addendum7=st_coeff*mod_psi*(p(7)*(-psi_x(i,j)-psi_y(i,j))**2/mod_psi_sq - b2)
                    addendum8=st_coeff*mod_psi*(p(8)*(psi_x(i,j)-psi_y(i,j))**2/mod_psi_sq - b2)
                    !recoloring
                    gaddendum1=0.25*beta*rho(i,j)*p(1)*(1.0_db-psi(i,j)**2)*psi_x(i,j)/mod_psi
                    gaddendum2=0.25*beta*rho(i,j)*p(2)*(1.0_db-psi(i,j)**2)*psi_y(i,j)/mod_psi
                    gaddendum3=0.25*beta*rho(i,j)*p(3)*(1.0_db-psi(i,j)**2)*(-psi_x(i,j)/mod_psi)
                    gaddendum4=0.25*beta*rho(i,j)*p(4)*(1.0_db-psi(i,j)**2)*(-psi_y(i,j)/mod_psi)
                    gaddendum5=0.25*beta*rho(i,j)*p(5)*(1.0_db-psi(i,j)**2)*(psi_x(i,j)/mod_psi + psi_y(i,j)/mod_psi)
                    gaddendum6=0.25*beta*rho(i,j)*p(6)*(1.0_db-psi(i,j)**2)*(-psi_x(i,j)/mod_psi + psi_y(i,j)/mod_psi)
                    gaddendum7=0.25*beta*rho(i,j)*p(7)*(1.0_db-psi(i,j)**2)*(-psi_x(i,j)/mod_psi - psi_y(i,j)/mod_psi)
                    gaddendum8=0.25*beta*rho(i,j)*p(8)*(1.0_db-psi(i,j)**2)*(psi_x(i,j)/mod_psi - psi_y(i,j)/mod_psi)
                endif
                !
                if(nci(i,j)==1 .and. nci(i-nint(psi_x(i,j)),j-nint(psi_y(i,j)))==0 &
                   .and. nci(i-2*nint(psi_x(i,j)),j-2*nint(psi_y(i,j)))==1)then
                    loc(i,j)=1
                endif
                !
                uu=0.5_db*(u(i,j)*u(i,j) + v(i,j)*v(i,j))/cssq
