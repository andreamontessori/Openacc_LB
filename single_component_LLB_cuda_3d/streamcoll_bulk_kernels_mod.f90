 ...
 module streamcoll_bulk_kernels
  ...
  attributes(global) subroutine streamcoll_bulk_shared()
    ...
    if(li==1)then
      !1 -1  0  0
	  udotc=u(i-1,j,k)*onecssq
	  f01(li-1,lj,lk)=p1*(rho(i-1,j,k)+(-halfonecssq*(u(i-1,j,k)*u(i-1,j,k) + v(i-1,j,k)*v(i-1,j,k) + w(i-1,j,k)*w(i-1,j,k))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !7 -1 -1  0
	  udotc=(u(i-1,j-1,k)+v(i-1,j-1,k))*onecssq
	  f07(li-1,lj-1,lk)=p2*(rho(i-1,j-1,k)+(-halfonecssq*(u(i-1,j-1,k)*u(i-1,j-1,k) + v(i-1,j-1,k)*v(i-1,j-1,k) + w(i-1,j-1,k)*w(i-1,j-1,k))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !9  -1 +1 0
      udotc=(-u(i-1,j+1,k)+v(i-1,j+1,k))*onecssq
	  f09(li-1,lj+1,lk)=p2*(rho(i-1,j+1,k)+(-halfonecssq*(u(i-1,j+1,k)*u(i-1,j+1,k) + v(i-1,j+1,k)*v(i-1,j+1,k) + w(i-1,j+1,k)*w(i-1,j+1,k))&
	   + half*udotc*udotc - udotc)) &
	  ...
	  !15  -1  0 -1
	  udotc=(u(i-1,j,k-1)+w(i-1,j,k-1))*onecssq
	  f15(li-1,lj,lk-1)=p2*(rho(i-1,j,k-1)+(-halfonecssq*(u(i-1,j,k-1)*u(i-1,j,k-1) + v(i-1,j,k-1)*v(i-1,j,k-1) + w(i-1,j,k-1)*w(i-1,j,k-1))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !18   -1   0  +1
	  udotc=(-u(i-1,j,k+1)+w(i-1,j,k+1))*onecssq
	  f18(li-1,lj,lk-1)=p2*(rho(i-1,j,k+1)+(-halfonecssq*(u(i-1,j,k+1)*u(i-1,j,k+1) + v(i-1,j,k+1)*v(i-1,j,k+1) + w(i-1,j,k+1)*w(i-1,j,k+1))&
	   + half*udotc*udotc - udotc)) &
    ...
    if(li==TILE_DIMx_d)then
      !2 +1  0  0
	  udotc=u(i+1,j,k)*onecssq
	  f02(li+1,lj,lk)=p1*(rho(i+1,j,k)+(-halfonecssq*(u(i+1,j,k)*u(i+1,j,k) + v(i+1,j,k)*v(i+1,j,k) + w(i+1,j,k)*w(i+1,j,k))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq1*(qxx*pxx(i+1,j,k)-cssq*(pyy(i+1,j,k)+pzz(i+1,j,k))) &
	   - fx*p1dcssq
	  !-1  0  0
	  !8 +1 +1  0
	  udotc=(u(i+1,j+1,k)+v(i+1,j+1,k))*onecssq
	  f08(li+1,lj+1,lk)=p2*(rho(i+1,j+1,k)+(-halfonecssq*(u(i+1,j+1,k)*u(i+1,j+1,k) + v(i+1,j+1,k)*v(i+1,j+1,k) + w(i+1,j+1,k)*w(i+1,j+1,k))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qxx*pxx(i+1,j+1,k)+qyy*pyy(i+1,j+1,k)-cssq*pzz(i+1,j+1,k)+two*qxy_7_8*pxy(i+1,j+1,k)) &
	   - (fx+fy)*p2dcssq
	  !-1 -1  0
	  ...
	  !10   +1 -1  0
	  udotc=(-u(i+1,j-1,k)+v(i+1,j-1,k))*onecssq
	  f10(li+1,lj-1,lk)=p2*(rho(i+1,j-1,k)+(-halfonecssq*(u(i+1,j-1,k)*u(i+1,j-1,k) + v(i+1,j-1,k)*v(i+1,j-1,k) + w(i+1,j-1,k)*w(i+1,j-1,k))&
	   + half*udotc*udotc + udotc)) &
	   +oneminusomega*pi2cssq2*(qxx*pxx(i+1,j-1,k)+qyy*pyy(i+1,j-1,k)-cssq*pzz(i+1,j-1,k)+two*qxy_9_10*pxy(i+1,j-1,k)) &
	   +(fy-fx)*p2dcssq
	  !-1 +1  0
	  ...
	  !16  +1  0 +1
	  udotc=(u(i+1,j,k+1)+w(i+1,j,k+1))*onecssq
	  f16(li+1,lj,lk+1)=p2*(rho(i+1,j,k+1)+(-halfonecssq*(u(i+1,j,k+1)*u(i+1,j,k+1) + v(i+1,j,k+1)*v(i+1,j,k+1) + w(i+1,j,k+1)*w(i+1,j,k+1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qxx*pxx(i+1,j,k+1)+qzz*pzz(i+1,j,k+1)-cssq*pyy(i+1,j,k+1)+two*qxz_15_16*pxz(i+1,j,k+1)) &
	   - (fx+fz)*p2dcssq
	  !-1  0  -1
	  ...
	  !17  +1  0 -1
	  udotc=(-u(i+1,j,k-1)+w(i+1,j,k-1))*onecssq
	  f17(li+1,lj,lk-1)=p2*(rho(i+1,j,k-1)+(-halfonecssq*(u(i+1,j,k-1)*u(i+1,j,k-1) + v(i+1,j,k-1)*v(i+1,j,k-1) + w(i+1,j,k-1)*w(i+1,j,k-1))&
	   + half*udotc*udotc + udotc)) &
	   + oneminusomega*pi2cssq2*(qxx*pxx(i+1,j,k-1)+qzz*pzz(i+1,j,k-1)-cssq*pyy(i+1,j,k-1)+two*qxz_17_18*pxz(i+1,j,k-1)) &
	   +(fz-fx)*p2dcssq
	  !-1  0  +1
    ...
    if(lj==1)then
      ...
      !3 0 -1  0
	  udotc=v(i,j-1,k)*onecssq
	  f03(li,lj-1,lk)=p1*(rho(i,j-1,k)+(-halfonecssq*(u(i,j-1,k)*u(i,j-1,k) + v(i,j-1,k)*v(i,j-1,k) + w(i,j-1,k)*w(i,j-1,k))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !11  0  -1  -1
	  udotc=(v(i,j-1,k-1)+w(i,j-1,k-1))*onecssq
	  f11(li,lj-1,lk-1)=p2*(rho(i,j-1,k-1)+(-halfonecssq*(u(i,j-1,k-1)*u(i,j-1,k-1) + v(i,j-1,k-1)*v(i,j-1,k-1) + w(i,j-1,k-1)*w(i,j-1,k-1))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !13  0  -1   +1
	  udotc=(v(i,j-1,k+1)-w(i,j-1,k+1))*onecssq
	  f13(li,lj-1,lk+1)=p2*(rho(i,j-1,k+1)+(-halfonecssq*(u(i,j-1,k+1)*u(i,j-1,k+1) + v(i,j-1,k+1)*v(i,j-1,k+1) + w(i,j-1,k+1)*w(i,j-1,k+1))&
	   + half*udotc*udotc + udotc)) &
    ...
    if(lj==TILE_DIMy_d)then
      ...
      !4  0 +1  0
	  udotc=v(i,j+1,k)*onecssq
	  f04(li,lj+1,lk)=p1*(rho(i,j+1,k)+(-halfonecssq*(u(i,j+1,k)*u(i,j+1,k) + v(i,j+1,k)*v(i,j+1,k) + w(i,j+1,k)*w(i,j+1,k))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq1*(qyy*pyy(i,j+1,k)-cssq*(pxx(i,j+1,k)+pzz(i,j+1,k))) &
	   - fy*p1dcssq
	  ! 0 -1  0
	  ...
	  !12  0  +1  +1
	  udotc=(v(i,j+1,k+1)+w(i,j+1,k+1))*onecssq
	  f12(li,lj+1,lk+1)=p2*(rho(i,j+1,k+1)+(-halfonecssq*(u(i,j+1,k+1)*u(i,j+1,k+1) + v(i,j+1,k+1)*v(i,j+1,k+1) + w(i,j+1,k+1)*w(i,j+1,k+1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qyy*pyy(i,j+1,k+1)+qzz*pzz(i,j+1,k+1)-cssq*pxx(i,j+1,k+1)+two*qyz_11_12*pyz(i,j+1,k+1)) &
	   - (fy+fz)*p2dcssq
	  ! 0 -1 -1
	  ...
	  !14  0  +1  -1
	  udotc=(v(i,j+1,k-1)-w(i,j+1,k-1))*onecssq
	  f14(li,lj+1,lk-1)=p2*(rho(i,j+1,k-1)+(-halfonecssq*(u(i,j+1,k-1)*u(i,j+1,k-1) + v(i,j+1,k-1)*v(i,j+1,k-1) + w(i,j+1,k-1)*w(i,j+1,k-1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qyy*pyy(i,j+1,k-1)+qzz*pzz(i,j+1,k-1)-cssq*pxx(i,j+1,k-1)+two*qyz_13_14*pyz(i,j+1,k-1)) &
	   + (fz-fy)*p2dcssq
	  ! 0 -1 +1
    ...
    if(lk==1)then
      ...
      !5  0  0 -1
	  udotc=w(i,j,k-1)*onecssq
	  f05(li,lj,lk-1)=p1*(rho(i,j,k-1)+(-halfonecssq*(u(i,j,k-1)*u(i,j,k-1) + v(i,j,k-1)*v(i,j,k-1) + w(i,j,k-1)*w(i,j,k-1))&
	   + half*udotc*udotc + udotc)) &
    ...
    if(lk==TILE_DIMz_d)then
      ...
      !6  0  0  +1
	  udotc=w(i,j,k+1)*onecssq
	  f06(li,lj,lk+1)=p1*(rho(i,j,k+1)+(-halfonecssq*(u(i,j,k+1)*u(i,j,k+1) + v(i,j,k+1)*v(i,j,k+1) + w(i,j,k+1)*w(i,j,k+1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq1*(qzz*pzz(i,j,k+1)-cssq*(pxx(i,j,k+1)+pyy(i,j,k+1))) &
	   - fz*p1dcssq
	  ! 0  0 -1
  ...
  attributes(global) subroutine streamcoll_bulk_shared_flop()
    ...
    if(li==1)then
      !1 -1  0  0
	  udotc=uh(i-1,j,k)*onecssq
	  f01(li-1,lj,lk)=p1*(rhoh(i-1,j,k)+(-halfonecssq*(uh(i-1,j,k)*uh(i-1,j,k) + vh(i-1,j,k)*vh(i-1,j,k) + wh(i-1,j,k)*wh(i-1,j,k))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !7 -1 -1  0
	  udotc=(uh(i-1,j-1,k)+vh(i-1,j-1,k))*onecssq
	  f07(li-1,lj-1,lk)=p2*(rhoh(i-1,j-1,k)+(-halfonecssq*(uh(i-1,j-1,k)*uh(i-1,j-1,k) + vh(i-1,j-1,k)*vh(i-1,j-1,k) + wh(i-1,j-1,k)*wh(i-1,j-1,k))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !9  -1 +1 0
      udotc=(-uh(i-1,j+1,k)+vh(i-1,j+1,k))*onecssq
	  f09(li-1,lj+1,lk)=p2*(rhoh(i-1,j+1,k)+(-halfonecssq*(uh(i-1,j+1,k)*uh(i-1,j+1,k) + vh(i-1,j+1,k)*vh(i-1,j+1,k) + wh(i-1,j+1,k)*wh(i-1,j+1,k))&
	   + half*udotc*udotc - udotc)) &
	  ...
	  !15  -1  0 -1
	  udotc=(uh(i-1,j,k-1)+wh(i-1,j,k-1))*onecssq
	  f15(li-1,lj,lk-1)=p2*(rhoh(i-1,j,k-1)+(-halfonecssq*(uh(i-1,j,k-1)*uh(i-1,j,k-1) + vh(i-1,j,k-1)*vh(i-1,j,k-1) + wh(i-1,j,k-1)*wh(i-1,j,k-1))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !18   -1   0  +1
	  udotc=(-uh(i-1,j,k+1)+wh(i-1,j,k+1))*onecssq
	  f18(li-1,lj,lk-1)=p2*(rhoh(i-1,j,k+1)+(-halfonecssq*(uh(i-1,j,k+1)*uh(i-1,j,k+1) + vh(i-1,j,k+1)*vh(i-1,j,k+1) + wh(i-1,j,k+1)*wh(i-1,j,k+1))&
	   + half*udotc*udotc - udotc)) &
    ...
    if(li==TILE_DIMx_d)then
      !2 +1  0  0
	  udotc=uh(i+1,j,k)*onecssq
	  f02(li+1,lj,lk)=p1*(rhoh(i+1,j,k)+(-halfonecssq*(uh(i+1,j,k)*uh(i+1,j,k) + vh(i+1,j,k)*vh(i+1,j,k) + wh(i+1,j,k)*wh(i+1,j,k))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq1*(qxx*pxxh(i+1,j,k)-cssq*(pyyh(i+1,j,k)+pzzh(i+1,j,k))) &
	   - fx*p1dcssq
	  !-1  0  0
	  !8 +1 +1  0
	  udotc=(uh(i+1,j+1,k)+vh(i+1,j+1,k))*onecssq
	  f08(li+1,lj+1,lk)=p2*(rhoh(i+1,j+1,k)+(-halfonecssq*(uh(i+1,j+1,k)*uh(i+1,j+1,k) + vh(i+1,j+1,k)*vh(i+1,j+1,k) + wh(i+1,j+1,k)*wh(i+1,j+1,k))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qxx*pxxh(i+1,j+1,k)+qyy*pyyh(i+1,j+1,k)-cssq*pzzh(i+1,j+1,k)+two*qxy_7_8*pxyh(i+1,j+1,k)) &
	   - (fx+fy)*p2dcssq
	  !-1 -1  0
	  ...
	  !10   +1 -1  0
	  udotc=(-uh(i+1,j-1,k)+vh(i+1,j-1,k))*onecssq
	  f10(li+1,lj-1,lk)=p2*(rhoh(i+1,j-1,k)+(-halfonecssq*(uh(i+1,j-1,k)*uh(i+1,j-1,k) + vh(i+1,j-1,k)*vh(i+1,j-1,k) + wh(i+1,j-1,k)*wh(i+1,j-1,k))&
	   + half*udotc*udotc + udotc)) &
	   +oneminusomega*pi2cssq2*(qxx*pxxh(i+1,j-1,k)+qyy*pyyh(i+1,j-1,k)-cssq*pzzh(i+1,j-1,k)+two*qxy_9_10*pxyh(i+1,j-1,k)) &
	   +(fy-fx)*p2dcssq
	  !-1 +1  0
	  ...
	  !16  +1  0 +1
	  udotc=(uh(i+1,j,k+1)+wh(i+1,j,k+1))*onecssq
	  f16(li+1,lj,lk+1)=p2*(rhoh(i+1,j,k+1)+(-halfonecssq*(uh(i+1,j,k+1)*uh(i+1,j,k+1) + vh(i+1,j,k+1)*vh(i+1,j,k+1) + wh(i+1,j,k+1)*wh(i+1,j,k+1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qxx*pxxh(i+1,j,k+1)+qzz*pzzh(i+1,j,k+1)-cssq*pyyh(i+1,j,k+1)+two*qxz_15_16*pxzh(i+1,j,k+1)) &
	   - (fx+fz)*p2dcssq
	  !-1  0  -1
	  ...
	  !17  +1  0 -1
	  udotc=(-uh(i+1,j,k-1)+wh(i+1,j,k-1))*onecssq
	  f17(li+1,lj,lk-1)=p2*(rhoh(i+1,j,k-1)+(-halfonecssq*(uh(i+1,j,k-1)*uh(i+1,j,k-1) + vh(i+1,j,k-1)*vh(i+1,j,k-1) + wh(i+1,j,k-1)*wh(i+1,j,k-1))&
	   + half*udotc*udotc + udotc)) &
	   + oneminusomega*pi2cssq2*(qxx*pxxh(i+1,j,k-1)+qzz*pzzh(i+1,j,k-1)-cssq*pyyh(i+1,j,k-1)+two*qxz_17_18*pxzh(i+1,j,k-1)) &
	   +(fz-fx)*p2dcssq
	  !-1  0  +1
    ...
    if(lj==1)then
      ...
      !3 0 -1  0
	  udotc=vh(i,j-1,k)*onecssq
	  f03(li,lj-1,lk)=p1*(rhoh(i,j-1,k)+(-halfonecssq*(uh(i,j-1,k)*uh(i,j-1,k) + vh(i,j-1,k)*vh(i,j-1,k) + wh(i,j-1,k)*wh(i,j-1,k))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !11  0  -1  -1
	  udotc=(vh(i,j-1,k-1)+wh(i,j-1,k-1))*onecssq
	  f11(li,lj-1,lk-1)=p2*(rhoh(i,j-1,k-1)+(-halfonecssq*(uh(i,j-1,k-1)*uh(i,j-1,k-1) + vh(i,j-1,k-1)*vh(i,j-1,k-1) + wh(i,j-1,k-1)*wh(i,j-1,k-1))&
	   + half*udotc*udotc + udotc)) &
	  ...
	  !13  0  -1   +1
	  udotc=(vh(i,j-1,k+1)-wh(i,j-1,k+1))*onecssq
	  f13(li,lj-1,lk+1)=p2*(rhoh(i,j-1,k+1)+(-halfonecssq*(uh(i,j-1,k+1)*uh(i,j-1,k+1) + vh(i,j-1,k+1)*vh(i,j-1,k+1) + wh(i,j-1,k+1)*wh(i,j-1,k+1))&
	   + half*udotc*udotc + udotc)) &
    ...
    if(lj==TILE_DIMy_d)then
      ...
      !4  0 +1  0
	  udotc=vh(i,j+1,k)*onecssq
	  f04(li,lj+1,lk)=p1*(rhoh(i,j+1,k)+(-halfonecssq*(uh(i,j+1,k)*uh(i,j+1,k) + vh(i,j+1,k)*vh(i,j+1,k) + wh(i,j+1,k)*wh(i,j+1,k))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq1*(qyy*pyyh(i,j+1,k)-cssq*(pxxh(i,j+1,k)+pzzh(i,j+1,k))) &
	   - fy*p1dcssq
	  ! 0 -1  0
	  ...
	  !12  0  +1  +1
	  udotc=(vh(i,j+1,k+1)+wh(i,j+1,k+1))*onecssq
	  f12(li,lj+1,lk+1)=p2*(rhoh(i,j+1,k+1)+(-halfonecssq*(uh(i,j+1,k+1)*uh(i,j+1,k+1) + vh(i,j+1,k+1)*vh(i,j+1,k+1) + wh(i,j+1,k+1)*wh(i,j+1,k+1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qyy*pyyh(i,j+1,k+1)+qzz*pzzh(i,j+1,k+1)-cssq*pxxh(i,j+1,k+1)+two*qyz_11_12*pyzh(i,j+1,k+1)) &
	   - (fy+fz)*p2dcssq
	  ! 0 -1 -1
	  ...
	  !14  0  +1  -1
	  udotc=(vh(i,j+1,k-1)-wh(i,j+1,k-1))*onecssq
	  f14(li,lj+1,lk-1)=p2*(rhoh(i,j+1,k-1)+(-halfonecssq*(uh(i,j+1,k-1)*uh(i,j+1,k-1) + vh(i,j+1,k-1)*vh(i,j+1,k-1) + wh(i,j+1,k-1)*wh(i,j+1,k-1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq2*(qyy*pyyh(i,j+1,k-1)+qzz*pzzh(i,j+1,k-1)-cssq*pxxh(i,j+1,k-1)+two*qyz_13_14*pyzh(i,j+1,k-1)) &
	   + (fz-fy)*p2dcssq
	  ! 0 -1 +1
    ...
    if(lk==1)then
      ...
      !5  0  0 -1
	  udotc=wh(i,j,k-1)*onecssq
	  f05(li,lj,lk-1)=p1*(rhoh(i,j,k-1)+(-halfonecssq*(uh(i,j,k-1)*uh(i,j,k-1) + vh(i,j,k-1)*vh(i,j,k-1) + wh(i,j,k-1)*wh(i,j,k-1))&
	   + half*udotc*udotc + udotc)) &
    ...
    if(lk==TILE_DIMz_d)then
      ...
      !6  0  0  +1
	  udotc=wh(i,j,k+1)*onecssq
	  f06(li,lj,lk+1)=p1*(rhoh(i,j,k+1)+(-halfonecssq*(uh(i,j,k+1)*uh(i,j,k+1) + vh(i,j,k+1)*vh(i,j,k+1) + wh(i,j,k+1)*wh(i,j,k+1))&
	   + half*udotc*udotc - udotc)) &
	   +oneminusomega*pi2cssq1*(qzz*pzzh(i,j,k+1)-cssq*(pxxh(i,j,k+1)+pyyh(i,j,k+1))) &
	   - fz*p1dcssq
	  ! 0  0 -1
